...
def tournaments_list_keyboard(
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text=t("page_prev", lang), callback_data=f"tournaments_page:{page - 1}"))
    if end < total:
        nav_row.append(InlineKeyboardButton(text=t("page_next", lang), callback_data=f"tournaments_page:{page + 1}"))
    if nav_row:
...
def tournament_detail_keyboard(
    ...
    *,
    ...
    buttons.append([InlineKeyboardButton(text=t("back", lang), callback_data="back_to_tournaments")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def admin_tournaments_keyboard(tournaments: list[tuple[UUID, str]], lang: str, *, action: str) -> InlineKeyboardMarkup:
    """action = 'edit' or 'delete'."""
    buttons = []
    for tid, name in tournaments:
        buttons.append([InlineKeyboardButton(text=name, callback_data=f"t_{action}:{tid}")])
    buttons.append([InlineKeyboardButton(text=t("cancel", lang), callback_data="t_cancel")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def currency_keyboard() -> InlineKeyboardMarkup:
    ...
    buttons = [InlineKeyboardButton(text=c, callback_data=f"currency:{c}") for c in currencies]
    return InlineKeyboardMarkup(inline_keyboard=[buttons])


def importance_keyboard() -> InlineKeyboardMarkup:
    buttons = [InlineKeyboardButton(text="⭐" * i, callback_data=f"importance:{i}") for i in range(1, 6)]
    return InlineKeyboardMarkup(inline_keyboard=[buttons])
...
def confirm_tournament_keyboard(lang: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=t("confirm", lang), callback_data="t_confirm_create"),
                InlineKeyboardButton(text=t("cancel", lang), callback_data="t_cancel"),
            ]
        ]
    )
