...
> Полный план работ по проекту. Обновлено: 2026-02-17 (v6 — API endpoints + error feedback + убрали моки в проде)
...
- [ ] **Роль определяется автоматически, не по выбору юзера** — если есть coach+athlete, всегда "coach"
  - `api/routes/me.py:22-30` — `_resolve_role()` не учитывает выбор юзера
  - Добавить `User.active_role` поле в модель, сохранять выбор при регистрации/переключении
...
- [ ] **Settings role switcher — admin-only**
- [ ] **Запрос смены роли** — форма как в онбординге → запрос админу
- [ ] **Админ: UI одобрения/отклонения** запросов смены роли
