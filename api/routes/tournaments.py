...
async def list_tournaments(
    ...
    age_category: str | None = Query(None, max_length=50),
...
async def enter_athletes(
    ...
    for athlete_id in data.athlete_ids:
        ...
        if not link_result.scalar_one_or_none():
            continue  # skip athletes not linked to coach
        ...
        # Get athlete data
        a_result = await ctx.session.execute(select(Athlete).where(Athlete.id == athlete_id))
        ...
        if tournament.weight_categories and athlete.weight_category not in tournament.weight_categories:
            continue  # skip athletes whose weight category doesn't match
...
@router.delete(
    "/tournaments/{tournament_id}/entries/{athlete_id}",
